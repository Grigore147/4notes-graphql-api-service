...
METADATA=$(cat <<EOF
{
    ...
    "images": {
        "nginx": "${CI_SERVICE_IMAGE_REPOSITORY_URL}/${CI_SERVICE_NAME}-nginx:${CI_SERVICE_VERSION}",
        "app": "${CI_SERVICE_IMAGE_REPOSITORY_URL}/${CI_SERVICE_NAME}-app:${CI_SERVICE_VERSION}"
    }
}
EOF
)
